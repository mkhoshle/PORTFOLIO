...
gfilter = torch.from_numpy(np.reshape(psf_heatmap, [1, 1, 7, 7])).to(device)
...
def L1L2loss(input_shape):
    def bump_mse(heatmap_true, spikes_pred):
        ...
        l1_loss = nn.L1Loss()
        loss_spikes = l1_loss(spikes_pred,torch.zeros(input_shape).to(device))
...
class NeuralNet(nn.Module):
    def __init__(self):
        ...
        self.bn1 = nn.BatchNorm2d(32,momentum=0.99,eps=0.001,track_running_stats=False)
        self.bn2 = nn.BatchNorm2d(64,momentum=0.99,eps=0.001,track_running_stats=False)
        self.bn3 = nn.BatchNorm2d(128,momentum=0.99,eps=0.001,track_running_stats=False)
        self.bn4 = nn.BatchNorm2d(512,momentum=0.99,eps=0.001,track_running_stats=False)
